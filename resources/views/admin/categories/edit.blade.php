...
@section('content')
<div class="animate-fade-in max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 overflow-hidden">
    <!-- Professional Header -->
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center space-x-3 sm:space-x-4">
                ...
                <div>
                    ...
                    <p class="text-xs sm:text-sm text-gray-600">Update category information and settings</p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 animate-slide-in-up">
        <div class="border-b border-gray-200 px-4 sm:px-6 py-4">
            <h3 class="text-base sm:text-lg font-semibold text-gray-900">Edit Category Information</h3>
            <p class="text-xs sm:text-sm text-gray-600 mt-1">Update the details below to modify your category</p>
        </div>
        
        <form method="POST" action="{{ route('admin.categories.update', $category) }}" enctype="multipart/form-data" class="p-4 sm:p-6">
            @csrf
            @method('PUT')
            ...
            <div class="max-w-2xl mx-auto">
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                    ...
                    <p class="text-xs text-blue-700 mt-1">Update the basic details of your category</p>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex items-center space-x-4">
                            <div class="flex-shrink-0 w-24 h-24 border-2 border-gray-300 border-dashed rounded-lg flex items-center justify-center bg-gray-50 overflow-hidden" id="image-preview">
                                @if($category->image)
                                    <img src="{{ asset('storage/' . $category->image) }}" alt="{{ $category->name }}" class="w-full h-full object-cover rounded-lg">
                                @else
                            ...
                            <div class="flex-1">
                                ...
                                <p class="mt-1 text-xs text-gray-500">Upload category image (JPG, PNG, GIF, WebP - Max 5MB). Leave empty to keep current image.</p>
                            </div>
                        </div>
                        @error('image')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <textarea name="description" 
                                  ...
                                  placeholder="Brief description (optional)">{{ old('description', $category->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
...
function previewImage(input) {
    ...
    if (input.files && input.files[0]) {
        ...
        @if($category->image)
            preview.innerHTML = `<img src="{{ asset('storage/' . $category->image) }}" alt="{{ $category->name }}" class="w-full h-full object-cover rounded-lg">`;
        @else
            preview.innerHTML = `
                <svg class="h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
